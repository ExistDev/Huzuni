...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onLivingUpdate() {
		// FastLadder
		if (Exist.moduleManager.getModule(FastLadder.class).isToggled()) {
			if (mc.thePlayer.isMoving() && mc.thePlayer.isOnLadder()) {
				mc.thePlayer.motionY += 0.05F;
			}
		}
		// Glide
		if (Exist.moduleManager.getModule(Glide.class).isToggled()) {
			if (Exist.settingManager.getSetting(Exist.moduleManager.getModule(Glide.class), "Mode").getCurrentOption()
		...
		// NoWeb
		if (Exist.moduleManager.getModule(WebWalk.class).isToggled()) {
			if (mc.thePlayer.isInWeb) {
				if (mc.thePlayer.onGround) {
					mc.thePlayer.motionY = (float) Exist.settingManager
							.getSetting(Exist.moduleManager.getModule(WebWalk.class), "WebUp").getCurrentValue();
				}
				setSpeed(1);
			}
		}

		if (Exist.moduleManager.getModule(WaterSpeed.class).isToggled()) {
		...
		if (Exist.moduleManager.getModule(InvMove.class).isToggled()) {
		...
		if (this.isUsingItem() && !this.isRiding()) {
			if (!Exist.moduleManager.getModule(NoSlowdown.class).isToggled()) {
