...
public class PlayModeButton extends GuiButton {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		...
		RenderHelper.drawRect(this.x, yPosition - height / 2, this.x + this.x1 + this.size + 12, yPosition + height,
				0xFF363539);
		RenderHelper.drawRect(this.x + this.x1 + this.size + 12, yPosition - height / 2,
				this.x + this.x1 + this.size + 14, yPosition + height, ColorUtils.getClientColor().hashCode());
		Exist.normalFontRenderer.font40.drawCenteredString(this.text, this.x + this.x1 / 2 + 4 + this.size,
				yPosition - height / 4, 0xFFFFFF);
	}
}
