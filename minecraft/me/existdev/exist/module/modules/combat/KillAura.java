...
public class KillAura extends Module {
	...
	public void AAC(EventPreMotionUpdates event) {
		...
		if (Exist.moduleManager.getModule(AutoArmor.class).isToggled() && AutoArmor.openedInventory) {
		...
		for (int k = 0; k < list.size(); k++) {
			if (((Entity) list.get(k)).getName() != mc.thePlayer.getName()) {
				...
				if (isValid(entityplayer)) {
					...
					C03PacketPlayer.yaw = this.getRotations(this.currentEntity)[0];
					C03PacketPlayer.pitch = this.getRotations(this.currentEntity)[1];
	...
	private boolean isValid(Entity e) {
		...
		if (e instanceof EntityLivingBase && f <= Exist.settingManager.getSetting(this, "Range").getCurrentValue()
				&& time.hasReached(
						(long) (Exist.settingManager.getSetting(this, "Delay").getCurrentValue() / 8) + randomInt(1, 9))
				&& !e.isDead && !mc.thePlayer.isDead && !e.isInvisible()) {
			time.reset();
			return true;
		} else {
			return false;
