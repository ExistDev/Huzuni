...
public class Speed extends Module {
	...
	private void NCPYPort() {
		MovementUtils p = null;
		if (mc.thePlayer.isMoving()) {
			p.setSpeed(p.getSpeed() + 0.0018);
			if (mc.thePlayer.onGround) {
				mc.thePlayer.jump();
			}
		}
	...
	public void onUpdate(EventPreMotionUpdates event) {
		...
		} else if (Exist.settingManager.getSetting(this, "Mode").getCurrentOption()
				.equalsIgnoreCase("NCPY-Portdwedasdsadsa")) {
		...
		if (!this.mc.thePlayer.isCollidedHorizontally && !this.mc.gameSettings.keyBindJump.pressed
			...
			if (this.mc.thePlayer.hurtTime <= 0) {
				if (this.mc.thePlayer.onGround) {
					...
					BlockPos willBe = new BlockPos(this.mc.thePlayer.posX, this.mc.thePlayer.posY - 0.6D,
							this.mc.thePlayer.posZ);
